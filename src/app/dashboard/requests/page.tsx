...
export default function PurchaseRequestPage() {
    ...
    const renderRequestCard = (request: PurchaseRequest) => {
        const {
            ...
            canRequestCancel, canCancelPending, canSendToReview, canSendToApproval, canGoBackToReview, canGoBackToPending
        ...
        const changeStatusActions = [
            ...
            { condition: canSendToApproval, action: () => actions.openStatusDialog(request, 'pending-approval'), label: 'Enviar a Aprobación', icon: <ShoppingBag className="mr-2"/>, className: 'text-orange-600' },
                     ...
                     <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-4 gap-y-6 text-sm">
                         ...
                         <div className="space-y-1">
                            ...
                            <Select value={request.priority} onValueChange={(value: PurchaseRequestPriority) => actions.handleDetailUpdate(request.id, { priority: value })}>
                                ...
                                <SelectContent>
                                    {Object.entries(selectors.priorityConfig).map(([key, {label}]) => <SelectItem key={key} value={key} disabled={!selectors.hasPermission('requests:edit:pending')}>{label}</SelectItem>)}
                                </SelectContent>
                            </Select>
                        </div>
                        <div className="space-y-1">
                        ...
                        {request.receivedDate && <div className="space-y-1"><p className="font-semibold text-muted-foreground">Fecha Recibida</p><p>{format(parseISO(request.receivedDate), 'dd/MM/yyyy HH:mm')}</p></div>}
                         ...
                         {request.analysis && typeof request.analysis === 'object' && 'cost' in request.analysis && (
                            <>
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 md:gap-4 flex-wrap">
                        ...
                        {selectors.hasPermission('requests:create') && (
                            <Dialog open={isNewRequestDialogOpen} onOpenChange={actions.setNewRequestDialogOpen}><DialogTrigger asChild><Button><FilePlus className="mr-2"/> Nueva Solicitud</Button></DialogTrigger><DialogContent className="sm:max-w-3xl"><form onSubmit={(e) => { e.preventDefault(); actions.handleCreateRequest(); }}><DialogHeader><DialogTitle>Crear Nueva Solicitud de Compra</DialogTitle><DialogDescription>Complete los detalles para crear una nueva solicitud.</DialogDescription></DialogHeader><ScrollArea className="h-[60vh] md:h-auto"><div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4">
