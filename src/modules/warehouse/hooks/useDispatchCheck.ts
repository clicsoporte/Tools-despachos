...
```
- src/app/dashboard/warehouse/dispatch-check/page.tsx:
    - Replace `proceedToNextStep();` with `updateState({ step: 'finished' });` to stop automatic advance.
    - Add a "Siguiente Documento" button to the "finished" step UI to manually advance to the next document in the container.
    - Correct the `handleGoBack` function to correctly redirect to `/dashboard/warehouse/dispatch-center` when a `containerId` is present.
    - Adjust the final screen to show the new "Siguiente Documento" button conditionally.

```xml
<changes>
  <description>Se ha corregido el flujo de navegación del Centro de Despacho. Se eliminó el avance automático a la siguiente factura, añadiendo un botón de control manual. También se solucionó el "flash" de la pantalla de verificación y se corrigió el botón de cancelar para que regrese a la lista de facturas del contenedor.</description>
  <change>
    <file>/src/modules/warehouse/hooks/useDispatchCheck.ts</file>
    <content><![CDATA[/**
 * @fileoverview Hook to manage the state and logic for the Dispatch Check module.
 */
'use client';
